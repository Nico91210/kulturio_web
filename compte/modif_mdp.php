...
<div class="flexbox-mdp">
    ...
    <form action="#" method="post">
        <input class="design" name="pass_actu" type="password" placeholder="Mot de passe Actuel" required /><br/><br/>
        <input class="btndesign" type="submit" value="Valider" />
    </form>
    <br/>
    <?php
        if(isset($_POST["pass_actu"])){
            if(password_verify($_POST["pass_actu"], $_SESSION['user']['mdp'])){ ?>
            <br/>
                <form action="#" method="post">
                    <label>Nouveau Mot de passe :</label><br/>
                    <input class="design" name="pass_new" type="password" placeholder="Nouveau Mot de passe" required /><br/><br/>
                    <input class="btndesign" type="submit" value="Valider" />
                </form><br/>
           <?php 
        }else{
    ...
    if(isset($_POST["pass_new"])){
     ...
     $mdpHash = password_hash($_POST["pass_new"], PASSWORD_DEFAULT);
     $sql =  'UPDATE user SET mdp = ? WHERE email = ?';
     $resul = $conn->prepare($sql);
     $resul->execute([$mdpHash, $email]);
     $sql =  'SELECT * from user where email="'.$_SESSION['user']['email'].'"AND mdp="'.$mdpHash.'"';
