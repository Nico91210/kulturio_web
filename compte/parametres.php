...
<html lang="fr">
<?php session_start();
include('../bdd/compte/score_sampl.php') ;
$Datetime = DateTime::createFromFormat('Y-m-d H:i:s', $_SESSION['user']['date_creation']);
$Dateconvertit = $Datetime->format('d/m/Y');
if(empty($_SESSION)){
    header('location:../index.php');
}
if (!empty($_SESSION)) {
    include("../bdd/amis/notifs_amis.php");
}
?>
<head>
  <meta charset="utf-8">
    ...
    <ul id="ul-nav">
        ...
        <?php if(!empty($_SESSION))  {
            ...
            <svg version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" width="40" height="40" viewBox="0 0 80.13 80.13" style="enable-background:new 0 0 80.13 80.13;" xml:space="preserve">
                            <g>
                                <path fill="#B22222" d="M48.355,17.922c3.705,2.323,6.303,6.254,6.776,10.817c1.511,0.706,3.188,1.112,4.966,1.112
...
<div class="flexbox-parametres">
        <div class="flexbox-div-parametres">
<br/>



        <img src="../data/avatars/<?php echo $_SESSION['user']['avatar'] ?>" width="100" />
        <p><?php echo $_SESSION['user']['pseudo']; ?></p>
        <p><?php echo $_SESSION['user']['email']; ?></p>
        <p>&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;<?php echo $Dateconvertit; ?>&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;</p>
        <p>Victoire à Kulturio : <?php if ($_SESSION['user']['victoire'] === NULL) {
                                        echo "0";
                                    } else {
                                        echo $_SESSION['user']['victoire'];
                                    } ?></p>
        <p>Score à Sampl'Quizz :
            <?php if (!empty($score)) { ?>
        </p>
        <div class="flexbox-div-parametresbg">
                <table>
                    <tr>
                        <th>Date</th>
                        <th>Score</th>
    ...
    <a style="color: black;"  href="../bdd/compte/supp_compte.php">Supprimer le compte</a><br /><br />
    <a style="color: black;" id="btn_form_update" href="#">Modifier Avatar</a><br /><br />

    <form id="form_update" action="../bdd/compte/update_compte.php" method="post" enctype="multipart/form-data">
        <input type="submit" class="biche" alt="biche" name="deer" value="" />
        <input type="submit" class="dauphin" alt="dauphin" name="dolphin" value="" />
        <input type="submit" class="lion" alt="lion" name="lion" value="" />
        <input type="submit" class="manchot" alt="manchot" name="penguin" value="" />
        <input type="submit" class="zebre" alt="zebre" name="zebra" value="" /><br/>
        <input type="submit" class="ninho" alt="ninho" name="ninho" value="" />
        <input type="submit" class="nepal" alt="nepal" name="nepal" value="" />
        <input type="submit" class="nekfeu" alt="nekfeu" name="nekfeu" value="" />
        <input type="submit" class="freeze" alt="freeze" name="freeze" value="" />
    </form>

    <a style="color: black;"  href="modif_mdp.php">Modifier son Mot de Passe</a><br /><br />
    <a style="color: black;"  href="deconnexion.php">Deconnexion</a><br /><br />
    </div>
</div>
        </body>
</html>